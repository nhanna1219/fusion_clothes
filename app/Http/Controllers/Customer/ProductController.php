...
class ProductController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('customer.products.index', compact('products', 'childCategories', 'parentCategories', 'q_categories', 'q_filters', 'query', 'range', 'sort'));
    }
...
//         $category = ProductCategory::where('name', $request->query('query'))->first();
//         if ($category) {
//             if ($category->parent_id === null) {
//                 $q_categories = $q_categories . ',' . $category->name;
//             } else {
//                 $q_filters = $q_filters . ',' . $category->name;
//             }
//         }
...
//             $products->whereHas('category', function ($query) use ($q_categories) {
//                 $query->whereHas('parent', function ($query) use ($q_categories) {
//                     $query->whereIn('product_categories.name', explode(',', $q_categories));
//                 });
//             });
//         } elseif (empty($q_categories) && !empty($q_filters)) {
//             $products->join('product_categories', 'products.category_id', '=', 'product_categories.id')
...
//         } elseif (!empty($q_categories) && !empty($q_filters)) {
//             $products->whereHas('category', function ($query) use ($q_filters, $q_categories) {
//                 $query->whereIn('product_categories.name', explode(',', $q_filters))
//                     ->whereHas('parent', function ($query) use ($q_categories) {
//                         $query->whereIn('product_categories.name', explode(',', $q_categories));
//                     });
//             });
...
//         $products = $products->whereBetween('products.price', array($min, $max))->orderBy('products.'.$sort)->paginate($size);
