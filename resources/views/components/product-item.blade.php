...
<div {{ $attributes->class([
    ...
    <a class="mx-3 mt-3 flex h-50 overflow-hidden rounded-xl relative z-10" href="{{ route('customer.products.details', $id) }}">
        ...
        <img src="{{ asset('storage/best-seller-ic.png') }}" class="absolute mt-2 ms-3" width="30" height="30" alt="">
        @endif

        <img class="object-cover h-64" src="{{ asset($image) ?? asset('storage/product_image.png') }}" alt="product image" />
    </a>
    <div class="mt-2 px-4 pb-3 w-full z-10">
        <a href="#">
            <h3 class="text-[12px] font-medium text-[#616161] my-1" id="product-type">
                {{ $type}}
            </h3>
        </a>
        <a href="#">
            <h5 class="tracking-tight font-bold text-base text-slate-900" id="product-name">{!! $name !!}</h5>
        </a>
        <div class="mt-2 mb-3 flex items-center justify-between">
            <p>
                <span class="text-base font-bold text-slate-900" id="product-price">${{ $price }}</span>
            </p>
        ...
        <div class="flex justify-between">
            <a href="{{ route('customer.products.details', $id) }}" class="flex items-center justify-center rounded-md bg-stone-950 px-3 py-2 text-center text-[0.6rem] font-medium text-white hover:bg-stone-800 focus:outline-none focus:ring-4 focus:ring-blue-300 transition duration-300 ease-in-out">
            ...
            <a href="{{ route('customer.products.details', $id) }}" class="flex items-center justify-center rounded-md bg-neutral-800 px-3 py-2 text-center text-[0.6rem] font-medium text-white hover:bg-neutral-600 focus:outline-none focus:ring-4 focus:ring-blue-300 transition duration-300 ease-in-out ml-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-4 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
